...
def configure_test_env(contrail_fab_path='/opt/contrail/utils', test_dir='/contrail-test'):
    ...
    test_node_hostname = ''
    if 'test_node' in env.roledefs.keys():
	with settings(host_string = env.roledefs['test_node'][0]), hide('everything'):
	    host_name = run('hostname')
	    test_node_hostname = host_name
	...
	if 'test_node' in env.roledefs.keys() and host_string in env.roledefs['test_node']:
	    role_dict = {'type': 'test_node', 'params':{'test_node_hostname': test_node_hostname}}
	    host_dict['roles'].append(role_dict)
